...
Function nollaus()
		...
		If (level = 4 Or level = 5 Or level = 6) Then
			stars = LoadAnimImage("gfx\bubbles.png",50,20,0,3)
			MaskImage stars,0,0,255
		Else
			stars = LoadAnimImage("gfx\stars.png",50,20,0,3)
			MaskImage stars,0,0,255
		EndIf
End Function
...
Function siirtymat()
	...
	Else If (MilliSecs() < aloitus_timer + 2000) Then
	        ...
	        If (cpu_vai_ei = 0) Then DrawImage aloitus3b,x3,y2,0
	        kello = MilliSecs() 
;		If (level = 11) Then hall_of_fame() : End
	EndIf


	If ((lights2 = 6 Or lights = 6 Or lights3 = 6) Or sekunnit = 0) Then
...
;				cpu_vai_ei = 4  :nollaus() ; (10)
;			EndIf
;		EndIf
;		If (cpu_vai_ei = 4) Then 
;			If (lights2 < lights And lights2 < lights3) Then
;				cpu_vai_ei = 4 :nollaus() ;TÄSSÄ OLI ALUNPERIN 10
;			EndIf
;		EndIf
	
;		If (cpu_vai_ei = 3) Then ;jos kaksi ihmispelaajaa ja CPU			
;			If (lights < lights2 And lights < lights3) Then
;				cpu_vai_ei = 4 : nollaus()
;			EndIf
;			If (lights2 < lights And lights2 < lights3) Then
;				cpu_vai_ei = 2 : nollaus()
;			EndIf
;		EndIf
		
;		If (cpu_vai_ei = 0) Then
;			If (lights < lights3) Then nollaus() : cpu_vai_ei = 4  ;(10);hjallis poistettu
;		EndIf

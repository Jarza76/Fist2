...
If (isku > 1) Then
        If (isku = 18 And (MilliSecs() > hyppy_timer + 90)) Then ;voltti eteen
                ...
                Else
					...
					If (suunta = 0) Then
						If (x < ukko_oikearaja) Then x = x + 30				
					Else
						If (x > ukko_vasenraja) Then x = x - 30
					EndIf
        ...
        If (isku = 4 And (MilliSecs() > hyppy_timer + 100)) Then ;potku ylˆs
        ...
        If ((isku = 3) And (MilliSecs() > ukko_timer + 40)) Then
                If (suunta = 0) Then
                         If (x<ukko_oikearaja) Then
                                x = x + 12
                                ukko_timer = MilliSecs()
                        EndIf
                   Else
                         If (x>ukko_vasenraja) Then
                                x = x - 12
                                ukko_timer = MilliSecs()
                        EndIf
                EndIf
...
If ((Not KeyDown(kavely_oikea)) And (Not KeyDown(kavely_vasen)) Or (KeyDown(n_control))) Then	;t‰st‰ alkaa iskut ja potkut
        If (MilliSecs() > liike_timer + 100) Then
                        ...
                        Else If (KeyDown(n_alas) And alas_nollaus = 0) Then				;Maha - k‰‰ntˆ
								alas_nollaus = 1
                        ...
                        Else If (KeyDown(n_vala)) Then				;Mahaisku.
                                ...
                                EndIf
								EndIf
                       Else If (KeyDown(n_vyla)) Then				;Isku ylˆs.
                                isku_frame = 5
                                isku = 1
								osuu = 1
								suunta = 1
                                ...
                                EndIf
                                aani_paalle = 4
                        Else If (KeyDown(n_oala)) Then				;Potku ylˆs.
								If (suunta = 1) Then
...
;K‰vely alkaa
                        If ((Not KeyDown(n_control)) And (JoyDown(2) = 0) And (KeyDown(kavely_oikea) Or (JoyXDir()=1)) And x < ukko_oikearaja) Then
;oikealle
								isku = 0
                                aani_paalle = 0
                        ...
                        If ((Not KeyDown(n_control)) And (JoyDown(2) = 0) And (KeyDown(kavely_vasen) Or (JoyXDir()=-1)) And x > ukko_vasenraja) Then  
;vasemmalle
								isku = 0
                                aani_paalle = 0
                                If (MilliSecs() > ukko_timer+100) Then
